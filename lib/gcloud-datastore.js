...
GcloudDataStoreConnector.prototype.toDatabaseData = function (model, data) {
   ...
   return _.map(data, function (data, name) {
      ...
      var excluded = property && property.index ? property.index === false : true;
      return {
         name              : name,
         value             : data,
...
GcloudDataStoreConnector.prototype.save = function (model, data, callback) {
  console.log('SAVE', arguments);

  return this.updateAttributes(model, data.id, data, {}, callback);
}


// GcloudDataStoreConnector.prototype.update = function (model, where, data, options, cb) {
